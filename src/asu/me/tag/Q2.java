...
public class Q2 extends Quadrant {

	@Override
	public PVector[] getQuadrantPoints(List<PVector> points) {
		PVector[] dPoints = new PVector[4];
		
		dPoints[0] = points.get(0);
		dPoints[1] = new PVector(points.get(1).x - (points.get(0).x/2), points.get(1).y);
		dPoints[2] = new PVector(points.get(2).x - (points.get(0).x/2), points.get(2).y - (points.get(2).y / 2));
		dPoints[3] = new PVector(points.get(3).x, points.get(2).y - (points.get(2).y / 2));
